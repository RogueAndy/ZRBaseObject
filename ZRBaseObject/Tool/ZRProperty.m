...
NSString *const Device_Unrecognized = @"iPhone";

NSString *ZRAppBuild(void) {
    return [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleVersion"];
}

NSString *ZRAppName(void) {
    return [[[NSBundle mainBundle] infoDictionary] valueForKey:@"CFBundleDisplayName"];
}

NSString *ZRAppVersion(void) {
    return [[[NSBundle mainBundle] infoDictionary] valueForKey:@"CFBundleShortVersionString"];
}

UIWindow *ZRKeyWindow(void) {
...
void ZRSystemSetting(void) {
    NSURL *url = [NSURL URLWithString:UIApplicationOpenSettingsURLString];
    [[UIApplication sharedApplication] openURL:url];
}

BOOL ZRIsRegisterRemote(void) {
    BOOL isRemoteNotify = false;
    if (ZR_ios_system >= 8.0) {
...
+ (UIImage *)getImageInVideo:(NSString *)videoURL {
    NSURL *tvideoURL = [NSURL URLWithString:videoURL];
    if (!tvideoURL || ![tvideoURL scheme]) {
        tvideoURL = [NSURL fileURLWithPath:videoURL];
    }
    
    AVURLAsset *asset = [[AVURLAsset alloc] initWithURL:tvideoURL options:nil];
...
+ (CGFloat)labelTextCountWidthWithLabelText:(NSString *)labelText {
